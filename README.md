...
**Nodal** — Kotlin Multiplatform Compose приложение для **полиномиальной интерполяции** табличных функций. Реализует семь классических методов и визуализирует полученные многочлены в реальном времени.
...
* **LaTeX-рендеринг:** итоговые многочлены отображаются в математической нотации.
...
* **Платформы:** Android (`androidApp/`), iOS (`iosApp/`), общий код в `shared/commonMain`.
* **Навигация:** Voyager (`navigator`, `screenmodel`, `transitions`).
* **DI:** Koin.
* **Архитектура:** MVVM + UDF, общий `MainStore` для глобального состояния.
* **Математика:** `kotlin-bignum` (`BigDecimal`).
* **Рендеринг формул:** `latex-renderer`.
* **Логирование:** Kermit.
...
Учебный проект (лабораторная работа №5 по численным методам).
